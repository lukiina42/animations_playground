...
(function flamingoSetup() {
    const killFlamingoButton =
        document.getElementById("shootFlamingo");
...
(function headerSetup() {
    document
        ...
        .addEventListener("click", function hideHeader(event) {
            const spriteMan =
    ...
    document
        .getElementById("spriteContainer")
        .addEventListener("click", function returnHeader(event) {
            const spriteMan =
                document.getElementById("spriteContainer");
...
(function shootingLadySetup() {
    const shootingLadyElement = document.getElementById(
        "shootingLadySelect"
    );
    const bulletElement = document.getElementById("bulletSelect");
    const killFlamingoButton =
        document.getElementById("shootFlamingo");
    killFlamingoButton.addEventListener(
        "click",
        function returnHeader(event) {
            shootingLadyElement.classList.add("visible");
            killFlamingoButton.style.display = "none";
            setTimeout(() => {
                document
                    .getElementById("bulletSelect")
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        document
                            .getElementById("flamingoWithEyes")
                            .classList.add("killed");
                        setTimeout(() => {
                            shootingLadyElement.classList.remove(
                                "visible"
                            );
                            bulletElement.classList.remove(
                                "displayBlock"
                            );
                        }, 1000);
                    }, 1000);
